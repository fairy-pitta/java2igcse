...
export class RecursionGuard {
    static convert(node, context) {
        ...
        try {
            // Use ConverterFactory to get the appropriate converter and convert the node
            return ConverterFactory.getConverter(node).convert(node, context);
        }
        finally {
            // クリーンアップ
